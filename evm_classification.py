...
def fit(x_train, y_train, params, i3d_features=None):
    ...
    for cl in np.unique(y_train):  # train one evm for each positive class
        ...
        evm = EVM.EVM(
            tailsize=params["tail_size"],
            cover_threshold=params["cover_threshold"],
...
def increment_evm(
    ...
    for cl in np.unique(
        extreme_vectors_labels
    ):  # train one evm for each extreme vector class
        print("training evm for extreme vector class", cl)
        # separate the positive class from the rest
        positives = [
            x for i, x in enumerate(extreme_vectors) if extreme_vectors_labels[i] == cl
        ]
        negatives = [
            x for i, x in enumerate(extreme_vectors) if extreme_vectors_labels[i] != cl
        ]
    ...
    for cl in np.unique(
        ...
        positives = [
            x for i, x in enumerate(new_train_features) if new_train_labels[i] == cl
        ]
        negatives = [
            x for i, x in enumerate(new_train_features) if new_train_labels[i] != cl
        ]
        ...
        if i3d_features is not None:
            positives_i3d = [
                x for i, x in enumerate(i3d_features) if new_train_labels[i] == cl
            ]
