...
def generate_ucf101_folds(FULL_LIST, params):
    ...
    for fold in range(params["n_folds"]):
        ...
        known_classes = np.random.choice(
            unique_classes, params["n_train_classes"], replace=False
        )
        ...
        unknown_classes = np.random.choice(
            available_classes,
            params["n_test_classes"] - params["n_train_classes"],
            replace=False,
        )
        # split known classes into training and test knowns. Split must ensure each group of subclasses remain united
        known_videos = [
            x.split(".")[0] for x in FULL_LIST if x.split("/")[0] in known_classes
        ]
        unknown_videos = [
            x.split(".")[0] for x in FULL_LIST if x.split("/")[0] in unknown_classes
        ]
        ...
        for cl in known_classes:
            videos_current_class = sorted(
                [x for x in known_videos if x.split("/")[0] == cl]
            )
...
def get_filenames(PATH_TO_SPLITS, train_file):
    full_list_path = PATH_TO_SPLITS + train_file
    ...
    while classes_left:
        if len(remaining_classes) >= params["max_classes"]:
            selected_classes = np.random.choice(
                remaining_classes,
                random.randint(params["min_classes"], params["max_classes"]),
                replace=False,
            ).tolist()
