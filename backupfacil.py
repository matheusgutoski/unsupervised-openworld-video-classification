...
def args_inc(argv=None,r_seed = None):
    ...
    parser.add_argument('--seed', type=int, default=r_seed,
...
if __name__ == '__main__':
	...
	parser.add_argument('--triplet_lr', help='triplet learning rate with sgd', type=float, default = 0.001)
	...
	parser.add_argument('--output_path', help='Where to output results', type=str, default = 'comparison_results/')
	...
	for tt in range(n_tasks):
		trn_load.append(tdata.DataLoader(trn_dset[tt], batch_size=batch_size, shuffle=True, num_workers=num_workers))
		val_load.append(tdata.DataLoader(val_dset[tt], batch_size=batch_size, shuffle=False, num_workers=num_workers))
		tst_load.append(tdata.DataLoader(tst_dset[tt], batch_size=batch_size, shuffle=False, num_workers=num_workers))
	...
	appr, ___, _____, _________, ____________________, net, device = args_inc(r_seed=params['seed'])
	...
	for t, (_, ncla) in enumerate(taskcla):
		...
		for ii in range(t+1):
			...
			for p,t in zip(preds,targ):			
				outputs = [torch.nn.functional.log_softmax(output, dim=1) for output in p]
		...
		evaluation.single_evaluation_clustering(test_features,all_targets,all_preds, params)
