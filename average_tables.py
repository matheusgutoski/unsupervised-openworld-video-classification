...
output_file = table_paths[0].split('/')[0] + '/' + table_paths[0].split('/')[1] + '/' + table_paths[0].split('/')[2] + '_averages.csv' 
...
for i in range(len(table_contents[0])):
	...
	for j in range(len(table_contents)):
		if 'task' in table_contents[j][i] or 'Task' in table_contents[j][i] or 'completeness' in table_contents[j][i] or table_contents[j][i] == '\n':
			print('copy this')
	...
	if not ('task' in table_contents[j][i] or 'Task' in table_contents[j][i] or 'completeness' in table_contents[j][i] or table_contents[j][i] == '\n'):
		print('results')
		for a in averages:
			output.write(str(a)+',')
		output.write('\n')
