...
def generate_train_val_splits(x_train, y_train, params): #split known classes into training and val knowns. Split must ensure each group of subclasses remain united
        ...
        merged_knowns_train, merged_knowns_val, merged_knowns_train_labels , merged_knowns_val_labels = train_test_split(merged_list, merged_list_labels, train_size = params['train_ratio'], random_state = params['seed'], shuffle = True, stratify = merged_list_labels)
...
class DataGenerator(keras.utils.Sequence):
    def __init__(self, list_IDs, class_dict, batch_size = 6,
                 n_classes = 101, flow = False, shuffle = True, is_training = True, num_frames = 64):
    ...
    def __data_generation(self, filenames, flow = False):
        ...
        new_labels_dict = {}
        for i,j in zip(np.unique(list(self.class_dict.values())), range(len(np.unique(list(self.class_dict.values()))))):
                #print(i)
