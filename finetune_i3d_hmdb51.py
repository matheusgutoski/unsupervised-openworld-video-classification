...
import keras
import numpy as np
from data_generator_i3d_hmdb51 import DataGenerator, generate_train_val_splits
...
def finetune(x_train, y_train, class_indexes, params):
    ...
    es = EarlyStopping(
        monitor="val_accuracy", mode="max", verbose=1, baseline=0.95, patience=0
    )
    ...
    )  # Setting multiprocessing to True causes problems with the current implementation.
...
def extract_features(
    old_model, x_train, y_train, x_test, y_test, class_indexes, params
):
    ...
    if params["model"] == "ucf101":
        ...
        intermediate_model.add(
            Model(inputs=model.input, outputs=model.get_layer("1x1_conv").input)
        )
    ...
    x_train_features = model.predict(
        train_generator,
...
def extract_features_single(
    old_model, x_train, y_train, class_indexes, params, original_n_classes
):
    NUM_CLASSES_TRAIN = np.unique(y_train).shape[0]
    if params["model"] == "ucf101":
...
def init_ti3d(params):
    ...
    model = Model(
        inputs=[anchor_input, positive_input, negative_input], outputs=merged_vector
    )
...
def finetune_triplet_net(x_train, int_y_train, x_test, params, warm_start_model=None):
    ...
    def generate_semihard_triplets(
        x,
        y,
        embeddings,
        margin=0.2,
        max_anchors_class=2000,
        max_negatives=500,
        use_hard_triplets=True,
        limit_triplets_class=8000,
    ):
        np.random.seed(2020)
        ...
        for un in unique_y:
            ...
            perm = np.random.permutation(current_class_data.shape[0])[
                :max_anchors_class
            ]
            ...
            anchor_positive_distances = cosine_distances(
                anchors_embeddings, positives_embeddings
            )
            anchor_negative_distances = cosine_distances(
                anchors_embeddings, negatives_embeddings
            )
            semihards = 0
            hards = 0
            counter = 0
                ...
                for p in range(positives.shape[0]):
                    for n in range(negatives.shape[0]):
                        ...
                        if ap_d < an_d and diff < margin:
                            semihards += 1
                            counter += 1
                            class_semihard_triplets.append(
                                [anchors[a], positives[p], negatives[n]]
                            )
                        if use_hard_triplets:
                            if ap_d > an_d:
                                hards += 1
                                counter += 1
                                class_hard_triplets.append(
                                    [anchors[a], positives[p], negatives[n]]
                                )
    ...
    for i in range(epochs):
        ...
        X_train = generate_semihard_triplets(
            x_train,
            int_y_train,
            embeddings,
            margin=params["margin"],
            max_anchors_class=500,
            max_negatives=500,
            use_hard_triplets=True,
        )
        ...
        hist = model.fit(
            ...
            y=Y_dummy,
            batch_size=params["triplet_batch_size"],
            epochs=1,
            shuffle=True,
        )
...
def old_finetune_triplet_net(x_train, y_train, x_test, y_test, params):
    ...
    print(x_train.shape)
    print(y_train)
    X_train, x_val = triplet.generate_triplet(
        x_train, y_train, testsize=0.2, ap_pairs=30, an_pairs=30
    )
